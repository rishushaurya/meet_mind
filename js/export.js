...
const exportsObj = {
  ...
  formatActionText(item, person, index) {
    // Assuming personIndex is needed to check state, we need it. 
    // We'll calculate it by looking up the person in the array to be safe, or just use '[ ]' if not found easily.
    let stateKey = '';
    if (app.state.result) {
  ...
  showEmailPreview(personIndex) {
    ...
    let bodyText = `Hi ${person.name},\\n\\nHere are your action items from the ${data.meeting_type || 'recent'} meeting:\\n\\n`;
  ...
  createEmailModal() {
    ...
    // Add some inline styles just for the modal since it wasn't strictly defined in CSS
    ...
    style.textContent = \`
      ...
      .modal-body { padding: 20px; font-family: 'JetBrains Mono', monospace; font-size: 13px; }
