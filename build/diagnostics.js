...
// Build chunk checks
for (let i = 0; i <= 6; i++) {
  const chunkFile = `build/0${i}_${['MANIFEST', 'SCAFFOLDING', 'UI_DESIGN', 'CORE_APP', 'INPUT_PROCESSING', 'API_ENGINE', 'DASHBOARD_EXPORTS'][i]}.md`;
  check(`File: ${chunkFile}`, fs.existsSync(path.join(ROOT, `build/0${i}_${['MANIFEST', 'SCAFFOLDING', 'UI_DESIGN', 'CORE_APP', 'INPUT_PROCESSING', 'API_ENGINE', 'DASHBOARD_EXPORTS'][i]}.md`)), `Create ${chunkFile}`);
}
...
if (fs.existsSync(path.join(ROOT, 'style.css'))) {
  ...
  check('CSS has color tokens', css.includes('--color-primary'), 'Add CSS custom properties to style.css');
  check('CSS has font tokens', css.includes('--font-heading'), 'Add font custom properties to style.css');
  check('CSS has spacing tokens', css.includes('--space-md'), 'Add spacing custom properties to style.css');
}
